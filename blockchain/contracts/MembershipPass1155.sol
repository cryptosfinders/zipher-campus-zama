...
interface ISplitPayout {
    function recordPayment(address payer) external payable;
}

/**
 * @title MembershipPass1155
 * @notice ERC-1155 contract that mints access passes for PushCampus courses in exchange for the Push Chain native token.
...
contract MembershipPass1155 is ERC1155Supply, AccessControl, Pausable, ReentrancyGuard {
    ...
    function createCourse(
        ...
        emit CourseCreated(courseId, priceWei, splitter, creator, duration, transferCooldown);
    }

    function setPrice(uint256 courseId, uint256 newPriceWei) external whenNotPaused {
        ...
        uint256 oldPrice = courseInfo.priceWei;
        courseInfo.priceWei = newPriceWei;

        emit CoursePriceUpdated(courseId, oldPrice, newPriceWei);
    }
        ...
        onlyRole(MARKETPLACE_ROLE)
    {
        ...
        if (courseInfo.priceWei != 0 && amountPaid < courseInfo.priceWei) {
            revert AmountMismatch(courseInfo.priceWei, amountPaid);
        }
