...
export const toggleMarketplaceListing = mutation({
  args: {
    groupId: v.id("groups"),
    address: v.string(),
    isListed: v.boolean(),
  },
  handler: async (ctx, { groupId, address, isListed }) => {
    ...
    // 🔒 Permission: only owner can toggle
...
export const setMarketplaceListing = mutation({
  args: {
    address: v.string(),
    groupId: v.id("groups"),
    isListed: v.boolean(),
  },
  handler: async (ctx, { address, groupId, isListed }) => {
    const user = await requireUserByWallet(ctx, address);
    ...
    // 🔐 Only paid groups can be listed
    if (group.price <= 0) {
      throw new Error("Free groups cannot be listed");
    }
...
export const toggleListing = mutation({
  args: {
    groupId: v.id("groups"),
    isListed: v.boolean()
  },
  handler: async (ctx, { groupId, isListed }) => {
    await ctx.db.patch(groupId, { isListed });
  }
});
