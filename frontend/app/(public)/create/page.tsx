  ...
  .superRefine((data, ctx) => {
    ...
    if (!isValidMediaReference(data.thumbnailUrl)) {
      ctx.addIssue({
        ...
        code: z.ZodIssueCode.custom,
        message:
          'Thumbnail must be either a Convex upload, a valid image URL, or a supported video link.'
      })
    }
    ...
    data.galleryUrls?.forEach((value, index) => {
      if (!isValidMediaReference(value)) {
        ctx.addIssue({
          ...
          code: z.ZodIssueCode.custom,
          message:
            'Each gallery item must be a Convex upload, an image URL, or a supported video link.'
        })
      }
    })
  })
