...
export function GroupDirectory() {
  ...
  const entries = useQuery(api.groups.directory, {}) as DirectoryEntry[] | undefined
  ...
  const allTags = useMemo(() => {
    ...
    for (const entry of entries) {
      for (const tag of entry.group.tags ?? []) tagSet.add(tag)
    }
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter(({ group }) => {
      ...
      return (
        ...
        (group.shortDescription ?? '').toLowerCase().includes(q)
      )
    })
    ...
    return (
      <div className="w-full rounded-xl border border-border/50 bg-card/60 px-6 py-20 text-center backdrop-blur-sm">
        <div className="mx-auto max-w-md space-y-4">
          ...
          <h3 className="text-xl font-bold text-foreground">No Communities Yet</h3>
          <p className="text-sm text-muted-foreground">
            There are no communities yet. Create one to kick-start your Zipher Campus.
          </p>
          <div className="pt-2">
    ...
    <div className="w-full space-y-10">
      ...
      <div className="space-y-6">
        <div className="space-y-3">
          <h2 className="text-4xl font-bold text-foreground">Discover Communities</h2>
          <p className="text-base text-muted-foreground">
            Explore active student communities or{' '}
            <button
